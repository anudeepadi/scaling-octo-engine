...
class LinkPreviewTest {
  static Future<void> testQuitxtUrls() async {
    ...
    DebugConfig.debugPrint('🔍 LinkPreviewTest: Testing ${testUrls.length} QuitTXT URLs...');
    
    for (int i = 0; i < testUrls.length; i++) {
      final url = testUrls[i];
      DebugConfig.debugPrint('');
      DebugConfig.debugPrint('📎 Testing URL ${i+1}/${testUrls.length}: $url');
  ...
  static Future<void> testBasicUrls() async {
    ...
    DebugConfig.debugPrint('🔍 LinkPreviewTest: Testing ${testUrls.length} basic URLs...');
